...
export default function App() {
  ...
  const handleEnterFullscreen = React.useCallback(() => setIsOptimizationFullscreen(true), []);
  const handleLeaveFullscreen = React.useCallback(() => setIsOptimizationFullscreen(false), []);
                    ...
                    {currentActiveTask ? (
                        ...
                        (() => {
                            switch (view) {
                                ...
                                case 'OPTIMIZATION': return <GroupOptimizationScreen products={products} ingredients={effectiveIngredients} nutrients={nutrients} isDynamicMatrix={isDynamicMatrix} selectedDietIds={selectedDietIds} onOpenInNewWindow={(data, name) => handleOpenTask('GROUP_OPTIMIZATION', name, data)} onUpdateProduct={(updatedProduct) => setProducts(prev => prev.map(p => p.id === updatedProduct.id ? updatedProduct : p))} setIsDirty={setIsDirty} savedFormulas={savedFormulas} setSavedFormulas={setSavedFormulas} onRemoveDietFromSelection={(id) => setSelectedDietIds(prev => prev.filter(d => d !== id))} onEnterFullscreen={handleEnterFullscreen} onLeaveFullscreen={handleLeaveFullscreen} onNavigate={setView} />;
