...
export const Header: React.FC<HeaderProps> = ({
    ...
    return (
        <header className="shrink-0 h-20 border-b border-gray-800/60 z-50 flex items-center px-6 gap-6 relative overflow-hidden bg-white">
            {/* Banner Táctico Sliced (Opción A): Logos fijos en extremos + Centro estirado */}
            <div className="absolute inset-0 flex pointer-events-none select-none overflow-hidden">
                {/* Extremo Izquierdo: Logo Gris Sutil */}
                <div 
                ...
                />
                {/* Centro: Estiramiento del Blanco */}
                <div className="flex-1 bg-white h-full" />
                {/* Extremo Derecho: Logo Azul */}
                <div 
                    className="w-1/3 h-full overflow-hidden"
                    style={{
                        backgroundImage: 'url("/banner feedpro.jpg")',
            ...
            {/* Nav con colores de alto contraste para fondo claro */}
                    ...
                    {activeTasks.map(task => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className={`text-[12px] font-black uppercase tracking-tight transition-colors ${isActive ? 'text-gray-900' : 'text-gray-500 group-hover:text-cyan-700'}`}>{task.name}</span>
                                <button
